...
async function exportApp(dir, options, configuration) {
    ...
    return nextExportSpan.traceAsyncFn(async ()=>{
        ...
        if (!(0, _fs).existsSync(buildIdFile)) {
            throw new Error(`Could not find a production build in the '${distDir}' directory. Try building your app with 'blitz build' before starting the static export. https://nextjs.org/docs/messages/next-export-no-build-id`);
        }
        ...
        if (outDir === (0, _path).join(dir, 'public')) {
            throw new Error(`The 'public' directory is reserved in Blitz.js and can not be used as the export out directory. https://nextjs.org/docs/messages/can-not-output-to-public`);
        }
        if (outDir === (0, _path).join(dir, 'static')) {
            throw new Error(`The 'static' directory is reserved in Blitz.js and can not be used as the export out directory. https://nextjs.org/docs/messages/can-not-output-to-static`);
        }
        ...
        if (typeof nextConfig.exportPathMap !== 'function') {
            if (!options.silent) {
                Log.info(`No "exportPathMap" found in "blitz.config.js". Generating map from "./pages"`);
            }
            nextConfig.exportPathMap = async (defaultMap)=>{
                return defaultMap;
            };
        }
        const { i18n , images: { loader ='default'  } , middleware ,  } = nextConfig;
        if (i18n && !options.buildExport) {
            throw new Error(`i18n support is not compatible with blitz export. See here for more info on deploying: https://nextjs.org/docs/deployment`);
        }
        if (!options.buildExport) {
            ...
            if (middleware === null || middleware === void 0 ? void 0 : middleware.length) {
                throw new Error(`Blitz http middleware is not compatible with \`blitz export\`.
  Possible solutions:
    ...
    - Remove \`middleware\` from \`blitz.config.js\`.\n`);
            ...
            if (isNextImageImported && loader === 'default' && !_ciInfo.hasNextSupport) {
                throw new Error(`Image Optimization using Blitz.js' default loader is not compatible with \`blitz export\`.
  Possible solutions:
  ...
  Read more: https://nextjs.org/docs/messages/export-image-api`);
        ...
        if (hasApiRoutes) {
            if (!options.silent) {
                Log.warn(_chalk.default.yellow(`Statically exporting a Blitz.js application via \`blitz export\` disables API routes.`) + `\n` + _chalk.default.yellow(`This command is meant for static-only hosts, and is` + ' ' + _chalk.default.bold(`not necessary to make your application static.`)) + `\n` + _chalk.default.yellow(`Pages in your application without server-side data dependencies will be automatically statically exported by \`blitz build\`, including pages powered by \`getStaticProps\`.`) + `\n` + _chalk.default.yellow(`Learn more: https://nextjs.org/docs/messages/api-routes-static-export`));
            }
        }
