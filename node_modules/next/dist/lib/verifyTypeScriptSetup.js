...
async function verifyTypeScriptSetup(dir, pagesDir, typeCheckPreflight, imageImportsEnabled, cacheDir) {
    ...
    try {
        ...
        if (_semver.default.lt(ts.version, '4.3.2') && !Boolean(process.env.BLITZ_TEST_ENVIRONMENT)) {
            log.warn(`Minimum recommended TypeScript version is v4.3.2, older versions can potentially be incompatible with Blitz.js. Detected: ${ts.version}`);
        }
