...
function isSerializableProps(page, method, input) {
    ...
    function isSerializable(refs, value, path) {
        ...
        type === 'boolean' || type === 'number' || type === 'string' || type === 'object' || type === 'bigint' || value === undefined || value instanceof Date || value instanceof Map || value instanceof Set || value instanceof RegExp || value instanceof Error) {
            return true;
        }
        if (type === 'undefined') {
        ...
        throw new SerializableError(page, method, path, '`' + type + '`' + '' + ' cannot be serialized as JSON. Please only return JSON serializable data types.');
    }
