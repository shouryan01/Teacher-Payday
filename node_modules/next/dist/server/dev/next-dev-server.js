...
class DevServer extends _nextServer.default {
    ...
    async startWatcher() {
        ...
        const regexPageExtension = (0, _utils3).buildPageExtensionRegex(this.nextConfig.pageExtensions);
        let resolved = false;
        return new Promise((resolve, reject)=>{
            ...
            wp.watch([], _utils3.topLevelFoldersThatMayContainPages.map((dir)=>(0, _path).join(pagesDir, dir)
            ), 0);
            wp.on('aggregated', ()=>{
                // Intentially don't await so it finishes in the background
                (0, _routes).saveRouteManifest(this.pagesDir, this.nextConfig);
                const routedPages = [];
                const knownFiles = wp.getTimeInfoEntries();
                for (const [filePath, { accuracy  }] of knownFiles){
                    const relativePath = '/' + (0, _path).relative(pagesDir, filePath);
                    if (accuracy === undefined || !(0, _utils3).getIsPageFile(relativePath) || !regexPageExtension.test(filePath)) {
                        continue;
                    }
                    let pageName = relativePath.replace(/\\+/g, '/');
                    pageName = (0, _utils3).convertPageFilePathToRoutePath(pageName, this.nextConfig.pageExtensions);
    ...
    async prepare() {
        await (0, _routes).saveRouteManifest(this.pagesDir, this.nextConfig);
