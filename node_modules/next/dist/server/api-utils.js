...
exports.getRpcMiddleware = getRpcMiddleware;
...
async function apiResolver(req, res, query, resolverModule, apiContext, propagateError, { route , config: blitzConfig  }) {
    const apiReq = req;
    const apiRes = res;
    try {
        var ref, ref1;
        ...
        if ((0, _utils).getIsRpcRoute(route)) {
            const middleware = (0, _middleware).getAndValidateMiddleware(blitzConfig, resolverModule, route);
            middleware.push(getRpcMiddleware(resolver, {
                route
            }));
            await (0, _middleware).handleRequestWithMiddleware(req, res, middleware);
        } else {
...
function getRpcMiddleware(resolver, { route  }) {
    return async (req, res, next)=>{
        const log = (0, _logging).baseLogger().getChildLogger({
            prefix: [
                route.replace('/api/rpc/', '') + '()'
            ]
        });
        const customChalk = new _chalk.default.constructor({
            level: log.settings.type === 'json' ? 0 : _chalk.default.level
        });
        if (req.method === 'HEAD') {
            ...
            try {
                ...
                const data = (0, _superjson).deserialize({
                    ...
                    meta: (ref = req.body.meta) === null || ref === void 0 ? void 0 : ref.params
                });
                ...
                const serializedResult = (0, _superjson).serialize(result);
                ...
                log.debug(customChalk.dim(`Next.js serialization:${(0, _utils1).prettyMs(Date.now() - nextSerializerStartTime)}`));
                const serializerDuration = Date.now() - serializerStartTime;
                const duration = Date.now() - startTime;
                log.info(customChalk.dim(`Finished: resolver:${(0, _utils1).prettyMs(resolverDuration)} serializer:${(0, _utils1).prettyMs(serializerDuration)} total:${(0, _utils1).prettyMs(duration)}`));
                (0, _logging).newline();
                return next();
            } catch (error) {
                ...
                const serializedError = (0, _superjson).serialize(error);
                res.json({
                    result: null,
