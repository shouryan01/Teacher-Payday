...
Object.defineProperty(exports, "BlitzConfig", {
    enumerable: true,
    get: function() {
        return _configShared.BlitzConfig;
    }
});
...
function assignDefaults(userConfig) {
    ...
    if (typeof userConfig.exportTrailingSlash !== 'undefined') {
        console.warn(_chalk.default.yellow.bold('Warning: ') + 'The "exportTrailingSlash" option has been renamed to "trailingSlash". Please update your blitz.config.js.');
    ...
    if (typeof ((ref = userConfig.experimental) === null || ref === void 0 ? void 0 : ref.reactMode) !== 'undefined') {
        var ref1;
        console.warn(_chalk.default.yellow.bold('Warning: ') + 'The experimental "reactMode" option has been replaced with "reactRoot". Please update your blitz.config.js.');
    ...
    const config = Object.keys(userConfig).reduce((currentConfig, key)=>{
        ...
        if (key === 'distDir') {
            ...
            if (userDistDir === 'public') {
                throw new Error(`The 'public' directory is reserved in Blitz.js and can not be set as the 'distDir'. https://nextjs.org/docs/messages/can-not-output-to-public`);
            }
...
async function loadConfig(phase, dir, customConfig) {
    (0, _env).loadEnvConfig(dir, phase === _constants.PHASE_DEVELOPMENT_SERVER, Log);
    if (![
        'start',
        's'
    ].includes(process.argv[2])) {
        // Do not compile config for blitz start because it was already compiled during blitz build
        await (0, _configShared).compileConfig(dir);
    }
    ...
    if (path === null || path === void 0 ? void 0 : path.length) {
        ...
        let userConfig = (0, _configShared).normalizeConfig(phase, userConfigModule.default || userConfigModule);
        if (process.env.VERCEL_BUILDER) {
            debug("Loading Vercel's next.config.js...");
            const nextConfig = require((0, _path).join('dir', 'next.config.js'));
        ...
        if (Object.keys(userConfig).length === 0) {
            Log.warn('Detected blitz.config.js, no exported configuration found. https://nextjs.org/docs/messages/empty-configuration');
        }
        ...
        const unsupportedPath = _findUp.default.sync([
            `blitz.config.jsx`,
            `blitz.config.tsx`,
            `blitz.config.json`,
            `next.config.jsx`,
        ...
        if (unsupportedPath === null || unsupportedPath === void 0 ? void 0 : unsupportedPath.length) {
            throw new Error(`Configuring Blitz.js via '${(0, _path).basename(unsupportedPath)}' is not supported. Please replace the file with 'blitz.config.(js|ts)'`);
        }
    }
