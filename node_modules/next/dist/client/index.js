...
const version = "11.1.0-0.45.3";
exports.version = version;
const looseToArray = (input)=>[].slice.call(input)
;
if (data.props) {
    data.props.pageProps = (0, _superjson).deserialize(data.props.superjsonProps || {
    });
}
...
if (process.env.NODE_ENV === 'development') {
    function onUnhandledError(ev) {
        if (ev.error instanceof _errors.RedirectError || ev.error instanceof _errors.AuthenticationError || ev.error instanceof _errors.AuthorizationError || ev.error instanceof _errors.NotFoundError) {
    ...
    if ('Cypress' in window) {
        ...
        window.onerror = (message, source, lineno, colno, err)=>{
            if (cypressOnErrorFun && !(err instanceof _errors.RedirectError || err instanceof _errors.AuthenticationError || err instanceof _errors.AuthorizationError || err instanceof _errors.NotFoundError)) {
...
function renderError(renderErrorProps) {
    ...
    return pageLoader.loadPage('/_error').then(({ page: ErrorComponent , styleSheets  })=>{
        return lastUserComponent === ErrorComponent ? Promise.resolve().then(function() {
...
function renderReactElement(domEl, fn) {
    ...
    if (process.env.__NEXT_REACT_ROOT) {
        // start blitz
        if (!reactRoot) {
            const createRootName = typeof _reactDom.default.unstable_createRoot === 'function' ? 'unstable_createRoot' : 'createRoot';
...
function AppContainer({ children , appProps  }) {
    ...
    }, /*#__PURE__*/ _react.default.createElement(_blitzAppRoot.BlitzWrapper, {
        appProps: appProps
...
;
let lastUserComponent;
let cachedWrappedComponent;
let lastAppProps;
function doRender(input) {
    ...
    if (Component === lastUserComponent) {
        ...
        cachedWrappedComponent = (0, _blitzAppRoot).withBlitzInnerWrapper(Component);
        Component = cachedWrappedComponent;
    }
    ...
    }), /*#__PURE__*/ _react.default.createElement(AppContainer, {
        appProps: appProps
    }, /*#__PURE__*/ _react.default.createElement(App, Object.assign({
