...
function defaultImageLoader(loaderProps) {
    ...
    throw new Error(`Unknown "loader" found in "blitz.config.js". Expected: ${_imageConfig.VALID_LOADERS.join(', ')}. Received: ${configLoader}`);
}
...
function Image1(_param) {
    ...
    if (process.env.NODE_ENV !== 'production') {
        if (!src) {
            throw new Error(`Image is missing required "src" property. Make sure you pass "src" in props to the \`<Image>\` component. Received: ${JSON.stringify({
                width,
                height,
                quality
            })}`);
...
function defaultLoader({ root , src , width , quality  }) {
    if (process.env.NODE_ENV !== 'production') {
        ...
        if (missingValues.length > 0) {
            throw new Error(`Next Image Optimization requires ${missingValues.join(', ')} to be provided. Make sure you pass them as props to the \`<Image>\` component. Received: ${JSON.stringify({
                src,
                width,
                quality
            })}`);
        ...
        if (src.startsWith('//')) {
            throw new Error(`Failed to parse src "${src}" on \`<Image>\`, protocol-relative URL (//) must be changed to an absolute URL (http:// or https://)`);
        }
        if (!src.startsWith('/') && configDomains) {
            ...
            try {
                ...
                console.error(err);
                throw new Error(`Failed to parse src "${src}" on \`<Image>\`, if using relative image it must start with a leading slash "/" or be an absolute URL (http:// or https://)`);
            }
            if (process.env.NODE_ENV !== 'test' && !configDomains.includes(parsedSrc.hostname)) {
                throw new Error(`Invalid src prop (${src}) on \`<Image>\`, hostname "${parsedSrc.hostname}" is not configured under images in your \`blitz.config.js\`\n` + `See more info: https://nextjs.org/docs/messages/next-image-unconfigured-host`);
            }
        }
    }
