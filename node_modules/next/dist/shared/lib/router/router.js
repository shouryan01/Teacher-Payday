...
var _lodashFrompairs = _interopRequireDefault(require("next/dist/compiled/lodash.frompairs"));
...
function normalizeApiRoute(path) {
    return (0, _normalizeTrailingSlash).normalizePathTrailingSlash(addBasePath(path));
}
...
function resolveHref(router, href, resolveAs) {
    ...
    if ((urlParts[0] || '').match(/(\/\/|\\)/)) {
        console.error(`Invalid href passed to router: ${urlAsString}, repeated forward-slashes (//) or backslashes \\ are not valid in the href`);
...
class Router {
    constructor(pathname1, query1, as1, { initialProps , pageLoader , App , wrapApp , Component: Component1 , err: err1 , subscription , isFallback , locale , locales , defaultLocale , domainLocales , isPreview  }){
        ...
        this.query = query1;
        this.params = extractRouterParams(query1, extractQueryFromAsPath(as1));
    ...
    async getRouteInfo(route, pathname, query, as, resolvedAs, routeProps) {
        try {
            ...
            if (props.superjsonProps) {
                props.pageProps = (0, _superjson).deserialize(props.superjsonProps || {
                });
            }
...
function extractRouterParams(routerQuery, asPathQuery) {
    return (0, _lodashFrompairs).default(Object.entries(routerQuery).filter(([key, value])=>typeof asPathQuery[key] === 'undefined' || !areQueryValuesEqual(value, asPathQuery[key])
    ));
}
