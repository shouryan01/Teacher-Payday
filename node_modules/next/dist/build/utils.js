...
exports.convertPageFilePathToResolverName = convertPageFilePathToResolverName;
exports.convertPageFilePathToResolverType = convertPageFilePathToResolverType;
exports.getIsPageFile = getIsPageFile;
...
exports.topLevelFoldersThatMayContainPages = void 0;
...
const topLevelFoldersThatMayContainPages = [
    'pages',
    'src',
    'app',
    'integrations', 
];
exports.topLevelFoldersThatMayContainPages = topLevelFoldersThatMayContainPages;
function convertPageFilePathToRoutePath(filePath, pageExtensions) {
    return filePath.replace(/^.*?[\\/]pages[\\/]/, '/').replace(/^.*?[\\/]api[\\/]/, '/api/').replace(/^.*?[\\/]queries[\\/]/, '/api/rpc/').replace(/^.*?[\\/]mutations[\\/]/, '/api/rpc/').replace(new RegExp(`\\.+(${pageExtensions.join('|')})$`), '');
}
const fileExtensionRegex = /\.([a-z]+)$/;
function convertPageFilePathToResolverName(filePathFromAppRoot) {
    return filePathFromAppRoot.replace(/^.*[\\/](queries|mutations)[\\/]/, '').replace(fileExtensionRegex, '');
}
function convertPageFilePathToResolverType(filePathFromAppRoot) {
...
function collectPages(directory, pageExtensions) {
    return (0, _recursiveReaddir).recursiveFindPages(directory, buildPageExtensionRegex(pageExtensions));
}
    ...
    toPrerender.forEach((entry)=>{
        ...
        if (typeof entry === 'string') {
            ...
            if (entry.locale && !(locales === null || locales === void 0 ? void 0 : locales.includes(entry.locale))) {
                throw new Error(`Invalid locale returned from getStaticPaths for ${page}, the locale ${entry.locale} is not specified in blitz.config.js`);
            }
