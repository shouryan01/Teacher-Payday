...
function getCacheCharacteristics(loaderOptions, source, filename) {
    ...
    const isPageFile = !isNextDist && (0, _utils).getIsPageFile(relativePathFromRoot);
    const isRpcFile = !isNextDist && (0, _utils).getIsRpcFile(relativePathFromRoot);
 ...
 */ function getPlugins(loaderOptions, cacheCharacteristics) {
    const { isServer , isPageFile , isRpcFile , isNextDist , hasModuleExports ,  } = cacheCharacteristics;
    ...
    const rpcClientConfigItem = !isServer && isRpcFile ? (0, _core).createConfigItem([
        require('../plugins/blitz-rpc-client')
    ], {
        type: 'plugin'
    }) : null;
    const rpcServerTransformConfigItem = isServer && isRpcFile ? (0, _core).createConfigItem([
        require('../plugins/blitz-rpc-server-transform')
    ], {
        type: 'plugin'
    }) : null;
    ...
    const fixNodeFileTraceItem = isServer && isPageFile ? (0, _core).createConfigItem([
 ...
 */ function getCustomBabelConfig(configFilePath) {
    ...
    throw new Error('The Blitz.js Babel loader does not support .mjs or .cjs config files.');
}
/**
 ...
 */ function getCacheKey(cacheCharacteristics) {
    const { isServer , isPageFile , isRpcFile , isNextDist , hasModuleExports , fileExt ,  } = cacheCharacteristics;
    const flags = 0 | (isServer ? 1 : 0) | (isPageFile ? 2 : 0) | (isNextDist ? 4 : 0) | (hasModuleExports ? 8 : 0) | (isRpcFile ? 16 : 0);
