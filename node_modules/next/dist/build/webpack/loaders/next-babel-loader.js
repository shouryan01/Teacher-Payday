...
var _index = _interopRequireDefault(require("./babel-loader/src/index"));
...
const customBabelLoader = (0, _index).default((babel)=>{
    const presetItem = babel.createConfigItem(nextBabelPreset, {
        type: 'preset'
    });
    const applyCommonJs = babel.createConfigItem(require('../../babel/plugins/commonjs'), {
        type: 'plugin'
    });
    const commonJsItem = babel.createConfigItem(require('next/dist/compiled/babel/plugin-transform-modules-commonjs'), {
        type: 'plugin'
    });
    ...
    return {
        customOptions (opts) {
            ...
            const filename = (0, _path).join(opts.cwd, 'noop.js');
            const loader = Object.assign(opts.cache ? {
                cacheDirectory: (0, _path).join(opts.distDir, 'cache', 'next-babel-loader'),
                cacheIdentifier: cacheKey + (opts.isServer ? '-server' : '') + '-new-polyfills' + (opts.development ? '-development' : '-production') + (opts.hasReactRefresh ? '-react-refresh' : '') + (opts.hasJsxRuntime ? '-jsx-runtime' : '') + JSON.stringify(babel.loadPartialConfig({
                    filename,
                    cwd: opts.cwd,
        ...
        config (cfg, { source , customOptions: { isServer , pagesDir , development , hasReactRefresh , hasJsxRuntime ,  } ,  }) {
            const filepath = this.resourcePath;
            const options = Object.assign({
            }, cfg.options);
            const relativePathFromRoot = filepath.replace(pagesDir, '');
            const isPageFile = (0, _utils).getIsPageFile(relativePathFromRoot);
            const isRpcFile = (0, _utils).getIsRpcFile(relativePathFromRoot);
...
//# sourceMappingURL=next-babel-loader.js.map