...
function createPagesMapping(pagePaths, pageExtensions) {
    const previousPages = {
    };
    const pages = pagePaths.reduce((result, pagePath)=>{
        let page = `${(0, _utils).convertPageFilePathToRoutePath(pagePath, pageExtensions).replace(/\\/g, '/')}`.replace(/\/index$/, '');
        let pageKey = page === '' ? '/' : page;
        if (pageKey in result) {
            (0, _log).warn(`Duplicate page detected. ${_chalk.default.cyan(previousPages[pageKey])} and ${_chalk.default.cyan(pagePath)} both resolve to ${_chalk.default.cyan(pageKey)}.`);
        } else {
...
function createEntrypoints(pages, target, buildId, previewMode, config, loadedEnvFiles, { pagesDir  }) {
    const client = {
    };
    const server = {
    };
