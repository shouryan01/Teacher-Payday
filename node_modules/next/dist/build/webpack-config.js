...
var _utils1 = require("../server/lib/utils");
...
async function getBaseWebpackConfig(dir, { buildId , config , dev =false , isServer =false , pagesDir , target ='server' , reactProductionProfiling =false , entrypoints , rewrites , isDevFallback =false  }) {
    ...
    const hasReactExperimental = Boolean(reactDomVersion) && reactDomVersion.includes('experimental') // blitz
    ;
    const hasReact18 = Boolean(reactDomVersion) && (_semver.default.gte(reactDomVersion, '18.0.0') || ((ref = _semver.default.coerce(reactDomVersion)) === null || ref === void 0 ? void 0 : ref.version) === '18.0.0') || hasReactExperimental // blitz
    ;
    var _reactRoot;
    const hasReactRoot = (_reactRoot = config.experimental.reactRoot) !== null && _reactRoot !== void 0 ? _reactRoot : hasReact18;
    ...
    /* ------ Blitz.js ------- */ const hasDbModule = (0, _fs).existsSync(_path.default.join(dir, 'db/index.js')) || (0, _fs).existsSync(_path.default.join(dir, 'db/index.ts'));
    const sessionCookiePrefix = (0, _utils1).getSessionCookiePrefix(config);
    /* ------ Blitz.js ------- */ // Webpack 5 can use the faster babel loader, webpack 5 has built-in caching for loaders
    ...
    const clientEntries = !isServer ? {
        ...
        [_constants1.CLIENT_STATIC_FILES_RUNTIME_POLYFILLS]: _path.default.join(_constants.NEXT_PROJECT_ROOT_DIST_CLIENT, 'polyfills.js')
    } : undefined;
    let typeScriptPath;
    try {
    ...
    async function handleExternals(context, request, dependencyType, getResolve) {
        ...
        if (isLocal) {
            ...
            const isNextExternal = /next[/\\]dist[/\\](shared|server)[/\\](?!lib[/\\]router[/\\]router)/.test(res);
            if (isNextExternal) {
            ...
            new _webpack.webpack.DefinePlugin({
                ...
                ...Object.keys(config.env).reduce((acc, key)=>{
                    if (/^(?:NODE_.+)|^(?:__.+)$/i.test(key)) {
                        throw new Error(`The key "${key}" under "env" in blitz.config.js is not allowed. https://nextjs.org/docs/messages/env-key-not-allowed`);
                    }
                    return {
                        ...acc,
                ...
                ...isServer ? {
                    ...
                    'process.env.__BLITZ_SESSION_COOKIE_PREFIX': JSON.stringify(sessionCookiePrefix)
                },
    ...
    if (_webpack.isWebpack5) {
        ...
        const cache = {
            ...
            version: `${"11.1.0-0.45.3"}|${configVars}`,
    ...
    if (typeof config.webpack === 'function') {
        ...
        if (!webpackConfig) {
            throw new Error('Webpack config is undefined. You may have forgot to return properly from within the "webpack" method of your blitz.config.js.\n' + 'See more info here https://nextjs.org/docs/messages/undefined-webpack-config');
        }
        ...
        if (typeof webpackConfig.then === 'function') {
            console.warn('> Promise returned in blitz config. https://nextjs.org/docs/messages/promise-in-next-config');
        }
    }
    ...
    var ref17;
    const hasUserCssConfig = (ref17 = (ref9 = webpackConfig.module) === null || ref9 === void 0 ? void 0 : ref9.rules.some((rule)=>canMatchCss(rule.test) || canMatchCss(rule.include)
    )) !== null && ref17 !== void 0 ? ref17 : false;
    if (hasUserCssConfig) {
        var ref18, ref19, ref20, ref21;
