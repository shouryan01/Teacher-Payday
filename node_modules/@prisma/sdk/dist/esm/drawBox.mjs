...
function drawBox({ title, width, height, str, horizontalPadding }) {
  ...
  const topLine = title ? chalk.grey(chars.topLeft + chars.horizontal) + " " + chalk.reset.bold(title) + " " + chalk.grey(chars.horizontal.repeat(width - title.length - 2 - 3) + chars.topRight) + chalk.reset() : chalk.grey(chars.topLeft + chars.horizontal) + chalk.grey(chars.horizontal.repeat(width - 3) + chars.topRight);
  ...
  const mappedLines = lines.slice(-height).map((l) => {
    ...
    return `${chalk.grey(chars.vertical)}${" ".repeat(horizontalPadding)}${chalk.reset(cliTruncate(l, width - 2))}${" ".repeat(paddingRight - horizontalPadding)}${chalk.grey(chars.vertical)}`;
  }).join("\n");
