...
var jsonValueToExpression = function jsonValueToExpression(value) {
  return typeof value === 'string' ? j.stringLiteral(value) : typeof value === 'number' ? j.numericLiteral(value) : typeof value === 'boolean' ? j.booleanLiteral(value) : value === null ? j.nullLiteral() : Array.isArray(value) ? j.arrayExpression(value.map(jsonValueToExpression)) : j.objectExpression(Object.entries(value).filter(function (entry) {
    return entry[1] !== undefined;
  }).map(function (_ref) {
    var key = _ref[0],
        value = _ref[1];
    return j.objectProperty(j.stringLiteral(key), jsonValueToExpression(value));
  }));
};
              ...
              if (((_element$elements$3 = element.elements[1]) == null ? void 0 : _element$elements$3.type) === 'ObjectExpression' && element.elements[1].properties.length > 0) {
                ...
                if (Array.isArray(item)) {
                  ...
                  for (var _key in item[1]) {
                    ...
                    obj.properties.push(j.objectProperty(j.stringLiteral(_key), jsonValueToExpression(value)));
                  }
...
function wrapBlitzConfig(program, functionName) {
  ...
  if (moduleExportsExpressions.length === 0) {
    var moduleExportExpression = j.expressionStatement(j.assignmentExpression('=', j.memberExpression(j.identifier('module'), j.identifier('exports')), j.callExpression(j.identifier(functionName), [j.objectExpression([])])));
    program.get().node.program.body.push(moduleExportExpression);
  } else if (moduleExportsExpressions.length === 1) {
    var moduleExportsExpression = moduleExportsExpressions.get();
    moduleExportsExpression.value.right = j.callExpression(j.identifier(functionName), [moduleExportsExpression.value.right]);
  } else {
    console.warn("There are multiple 'module.exports' inside 'blitz.config.js'");
  }
...
export { type$4 as AddDependencyType, type$3 as FileTransformType, Frontmatter, type$2 as NewFileType, type$1 as PrintMessageType, RecipeBuilder, RecipeExecutor, addBabelPlugin, addBabelPreset, addBlitzMiddleware, addImport, addPrismaEnum, addPrismaField, addPrismaGenerator, addPrismaModel, addPrismaModelAttribute, customTsParser, findModuleExportsExpressions, getExecutorArgument, paths, produceSchema, setPrismaDataSource, transformBlitzConfig, withComments, withTypeAnnotation, wrapBlitzConfig };
