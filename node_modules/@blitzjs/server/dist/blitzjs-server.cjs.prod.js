...
var utils = require('next/dist/server/lib/utils');
...
async function resolveBinAsync(pkg, executable) {
  ...
  var _await$readJSON = await fsExtra.readJSON(path__default['default'].join(packageDir, "package.json")),
  ...
  var fullPath = path__default['default'].join(packageDir, binPath);
...
async function getIsTypeScript(rootFolder) {
  try {
    await fs.promises.access(path.join(rootFolder, "tsconfig.json"));
    return true;
  } catch (_unused) {
    return false;
  }
...
async function nextStartDev(nextBin, cwd, _manifest, _buildFolder, config) {
  ...
  return new Promise(function (res, rej) {
    if (config.port && availablePort !== config.port) {
      ...
      crossSpawn.spawn(nextBin, spawnCommand, {
        cwd: cwd,
        env: spawnEnv,
        stdio: "inherit"
...
function nextBuild(nextBin, _buildFolder, _manifest, config) {
  var spawnEnv = getSpawnEnv(config);
  return new Promise(function (res, rej) {
    crossSpawn.spawn(nextBin, ["build"], {
      env: spawnEnv,
      stdio: "inherit"
    }).on("exit", function (code) {
      if (code === 0 || code === null) {
...
function nextExport(nextBin, config) {
  var spawnEnv = getSpawnEnv(config);
  return new Promise(function (res, rej) {
    crossSpawn.spawn(nextBin, ["export"], {
      env: spawnEnv,
      stdio: "inherit"
    }).on("exit", function (code) {
      if (code === 0 || code === null) {
...
function getCustomServerBuildPath() {
  var projectRoot = utils.getProjectRootSync();
  return path__default['default'].resolve(projectRoot, ".next", "custom-server.js");
}
    ...
    esbuildOptions.watch = {
      onRebuild: function onRebuild(error) {
        if (error) {
          ...
          logging.newline();
          logging.log.progress("Custom server changed - rebuilding...");
        }
      }
    };
  }
...
function startCustomServer(_cwd, config, _temp2) {
  ...
  return new Promise(function (res, rej) {
    ...
    var spawnServer = function spawnServer() {
      process = crossSpawn.spawn("node", [serverBuildPath], {
        env: spawnEnv,
        stdio: "inherit"
      }).on("exit", function (code) {
        if (code === 0) {
    ...
    esbuildOptions.watch = watch ? {
      onRebuild: function onRebuild(error) {
        if (error) {
          ...
          logging.newline();
          logging.log.progress("Custom server changed - restarting...");
          logging.newline(); //@ts-ignore -- incorrect TS type from node
...
async function dev(config) {
  ...
  if (customServerExists()) {
    ...
    logging.log.success("Using your custom server");
