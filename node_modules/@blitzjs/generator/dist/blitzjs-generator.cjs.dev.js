...
var Generator = /*#__PURE__*/function (_EventEmitter) {
  ...
  _proto.process = function process(input, pathEnding, templateValues, prettierOptions) {
    ...
    if (!this.useTs && tsExtension.test(pathEnding)) {
      ...
      templatedFile = ((_babel$transform = babel.transform(templatedFile, {
        configFile: false,
  ...
  _proto.sourcePath = function sourcePath() {
    ...
    if (this.sourceRoot.type === "absolute") {
      ...
      return path.join.apply(path__namespace, [__dirname, process.env.NODE_ENV === "test" ? "../templates" : "./templates", this.sourceRoot.path].concat(paths));
    }
  };
...
var logFailedVersionFetch = function logFailedVersionFetch(dependency, fallback) {
  logging.log.clearLine();
  logging.baseLogger({
    displayDateTime: false
  }).warn("Failed to fetch latest version of '" + chalk__default['default'].bold(dependency) + "', falling back to '" + chalk__default['default'].bold(fallback) + "'.");
};
...
var AppGenerator = /*#__PURE__*/function (_Generator) {
  ...
  _proto.postWrite = async function postWrite() {
    ...
    var spinner = logging.log.spinner(logging.log.withBrand("Retrieving the freshest of dependencies")).start();
    ...
    if (!fallbackUsed && !this.options.skipInstall) {
      ...
      await new Promise(function (resolve) {
        ...
        var cp = spawn__default['default'](pkgManager, ["install", logFlag], {
        ...
        if (pkgManager !== "yarn") {
          var _spinner = logging.log.spinner(logging.log.withBrand("Installing those dependencies (this will take a few minutes)")).start();
        ...
        (_cp$stdout2 = cp.stdout) == null ? void 0 : _cp$stdout2.on("data", function (data) {
          if (pkgManager === "yarn") {
            ...
            if (json && json.type === "step") {
              ...
              var _spinner2 = logging.log.spinner(logging.log.withBrand(json.data.message)).start();
      ...
      var runLocalNodeCLI = function runLocalNodeCLI(command) {
        ...
        if (pkgManager === "yarn") {
          ...
          return spawn__default['default'].sync("pnpx", command.split(" "));
        } else {
      ...
      if (this.packageInstallSuccess) {
        var formattingSpinner = logging.log.spinner(logging.log.withBrand("Formatting your code")).start();
      ...
      if (this.options.skipInstall) {
        ...
        spinner.fail(chalk__default['default'].red.bold("We had some trouble connecting to the network, so we'll skip installing your dependencies right now. Make sure to run " + (this.pkgManager + " install") + " once you're connected again."));
      }
    }
  ...
  _proto.commitChanges = function commitChanges() {
    var commitSpinner = logging.log.spinner(logging.log.withBrand("Committing your app")).start();
...
var Field = /*#__PURE__*/function () {
  ...
  Field.parse = function parse(input, schema) {
    ...
    try {
      ...
      if (err instanceof MissingFieldNameError) {
        throw new Error("Each field in a model must have a name, but you supplied " + logging.log.variable(input) + ".\n         Try giving the field as a " + logging.log.variable("name:type") + " pair, such as " + logging.log.variable("description:string") + ".");
      }
  ...
  function Field(name, options) {
    ...
    if (!options.type) {
      logging.baseLogger({
        displayDateTime: false
      }).warn("No field type specified for field " + logging.log.variable(name) + ", falling back to " + logging.log.variable("String") + ".");
...
var getTemplateRoot = function getTemplateRoot(templateDir, fallback) {
  ...
  var templatePath = path.join(templateDir, fallback.path);
...
var ModelGenerator = /*#__PURE__*/function (_Generator) {
  ...
  _proto.write = async function write() {
    ...
    if (model) {
      logging.newline();
      logging.log.success("Model '" + modelDefinition.name + "'" + (dryRun ? "" : " " + updatedOrCreated + " in schema.prisma") + ":\n");
      ast.printSchema({
        type: "schema",
        list: [model]
      }).split("\n").map(logging.log.progress);
      logging.newline();
    }
  };
...
var PageGenerator = /*#__PURE__*/function (_Generator) {
  ...
  _proto.postWrite = async function postWrite() {
    var _await$import = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('next/dist/server/config-shared')); }),
        loadConfigProduction = _await$import.loadConfigProduction;

    var _await$import2 = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('next/dist/build/routes')); }),
