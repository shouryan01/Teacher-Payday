...
import { existsSync, readFileSync, ensureDir, readJSONSync, writeJson } from 'fs-extra';
...
var ConflictChecker = /*#__PURE__*/function (_Transform) {
  ...
  _proto._transform = function _transform(file, _encoding, cb) {
    ...
    var filePath = resolve(file.path);
  ...
  _proto.checkDiff = async function checkDiff(file) {
    ...
    var oldFileContents = readFileSync(resolve(file.path)).toString();
    var diff = diffLines(oldFileContents, newFileContents);
    var conflict = diff.some(function (line) {
    ...
    if (conflict) {
      ...
      do {
        ...
        if (((_answer2 = answer) == null ? void 0 : _answer2.action) === "show") this.printDiff(diff);
...
async function readdirRecursive(root, filter, dir) {
  ...
  var absoluteDir = path__default.resolve(root, dir);
  var dirStats = await promises.stat(absoluteDir);

  if (dirStats.isDirectory()) {
    var entries = await promises.readdir(absoluteDir);
    ...
    var recursiveList = await Promise.all(entries.map(function (name) {
      return readdirRecursive(root, filter, path__default.join(dir, name));
    }));
...
function replaceConditionalStatements(program, templateValues) {
  ...
  program.find(j.IfStatement, processEnvRequirements).forEach(function (path) {
    return replaceConditionalNode(path, templateValues);
  });
  program.find(j.ConditionalExpression, processEnvRequirements).forEach(function (path) {
    return replaceConditionalNode(path, templateValues);
  });
  return program;
}
...
function replaceJsxConditionals(program, templateValues) {
  program.find(j.JSXIdentifier, {
    name: "if"
  }).forEach(function (path) {
    if (j.JSXOpeningElement.check(path.parent.node)) {
      var conditionPath = j(path.parent).find(j.JSXAttribute, {
        name: {
          name: "condition"
        }
      }).at(0);
      ...
      var innerElements = path.parent.parent.node.children.filter(j.JSXElement.check.bind(j.JSXElement));
      var consequent = innerElements[0];
      var alternate = innerElements[1] && j(innerElements[1]).paths()[0].node.children.filter(j.JSXElement.check.bind(j.JSXElement));
...
var Generator = /*#__PURE__*/function (_EventEmitter) {
  ...
  function Generator(options) {
    ...
    _this.options = options;
    _this.store = create();
    _this.fs = create$1(_this.store);
    _this.enquirer = new enquirer();
    _this.useTs = typeof _this.options.useTs === "undefined" ? existsSync(resolve("tsconfig.json")) : _this.options.useTs;
  ...
  _proto.process = function process(input, pathEnding, templateValues, prettierOptions) {
    ...
    if (!this.useTs && tsExtension.test(pathEnding)) {
      ...
      templatedFile = ((_babel$transform = transform(templatedFile, {
        configFile: false,
        plugins: [[babelTransformTypescript, {
          isTSX: true
        }]]
    ...
    if (codeFileExtensions.test(pathEnding) && typeof templatedFile === "string" && this.prettier && !this.prettierDisabled) {
      ...
      try {
        ...
        }).warn("Failed trying to run prettier: " + error);
      }
    }
  ...
  _proto.write = async function write() {
    ...
    var paths = await readdirRecursive(sourcePath, function (name) {
    ...
    var prettierOptions = await ((_this$prettier = this.prettier) == null ? void 0 : _this$prettier.resolveConfig(sourcePath));
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(paths), _step; !(_step = _iterator()).done;) {
      ...
      try {
        await async function () {
          ...
          pathSuffix = join(_this2.getTargetDirectory(), pathSuffix);
        ...
        baseLogger({
          displayDateTime: false
        }).error("Error generating " + filePath);
        throw error;
      }
    }
  };
  ...
  _proto.sourcePath = function sourcePath() {
    ...
    if (this.sourceRoot.type === "absolute") {
      return join.apply(path, [this.sourceRoot.path].concat(paths));
    } else {
      return join.apply(path, [__dirname, process.env.NODE_ENV === "test" ? "../templates" : "./templates", this.sourceRoot.path].concat(paths));
    }
  };
  ...
  _proto.destinationPath = function destinationPath() {
    ...
    return join.apply(path, [this.options.destinationRoot].concat(paths));
  };
  ...
  _proto.run = async function run() {
    ...
    if (!this.options.dryRun) {
      await ensureDir(this.options.destinationRoot);
...
var fetchAllVersions = async function fetchAllVersions(dependency) {
  var res = await got("https://registry.npmjs.org/" + dependency, {
    retry: {
      limit: 3
    },
    timeout: 3000,
...
var fetchDistTags = async function fetchDistTags(dependency) {
  var res = await got("https://registry.npmjs.org/-/package/" + dependency + "/dist-tags", {
    retry: {
      limit: 3
    },
    timeout: 3000,
...
var logFailedVersionFetch = function logFailedVersionFetch(dependency, fallback) {
  ...
  baseLogger({
    displayDateTime: false
  }).warn("Failed to fetch latest version of '" + chalk.bold(dependency) + "', falling back to '" + chalk.bold(fallback) + "'.");
};
...
var AppGenerator = /*#__PURE__*/function (_Generator) {
  ...
  function AppGenerator() {
    ...
    _this.sourceRoot = {
      type: "template",
      path: _this.options.template.path
    };
  ...
  _proto.filesToIgnore = function filesToIgnore() {
    if (!this.options.useTs) {
      return ["tsconfig.json", "blitz-env.d.ts", "jest.config.ts", "package.ts.json", "pre-push-ts", "types.ts"];
    }
  ...
  _proto.preCommit = async function preCommit() {
    ...
    this.fs.move(this.destinationPath(this.options.useTs ? ".husky/pre-push-ts" : ".husky/pre-push-js"), this.destinationPath(".husky/pre-push"));
  ...
  _proto.postWrite = async function postWrite() {
    ...
    if (!this.options.skipGit) {
      var initResult = spawn.sync("git", ["init"], {
        stdio: "ignore"
      });
      ...
      if (!gitInitSuccessful) {
        baseLogger({
          displayDateTime: false
        }).warn("Failed to run git init.");
        baseLogger({
          displayDateTime: false
        }).warn("Find out more about how to install git here: https://git-scm.com/downloads.");
      }
    }

    var pkgJsonLocation = join(this.destinationPath(), "package.json");
    var pkg = readJSONSync(pkgJsonLocation);
    console.log(""); // New line needed

    var spinner = log.spinner(log.withBrand("Retrieving the freshest of dependencies")).start();
    ...
    await writeJson(pkgJsonLocation, pkg, {
      spaces: 2
    });
    ...
    if (!fallbackUsed && !this.options.skipInstall) {
      ...
      await new Promise(function (resolve) {
        ...
        var logFlag = pkgManager === "yarn" ? "--json" : "--loglevel=error";
        var cp = spawn(pkgManager, ["install", logFlag], {
        ...
        if (pkgManager !== "yarn") {
          var _spinner = log.spinner(log.withBrand("Installing those dependencies (this will take a few minutes)")).start();
        ...
        (_cp$stdout2 = cp.stdout) == null ? void 0 : _cp$stdout2.on("data", function (data) {
          if (pkgManager === "yarn") {
            ...
            if (json && json.type === "step") {
              ...
              var _spinner2 = log.spinner(log.withBrand(json.data.message)).start();
      ...
      var runLocalNodeCLI = function runLocalNodeCLI(command) {
        var pkgManager = _this2.pkgManager;

        if (pkgManager === "yarn") {
          return spawn.sync("yarn", ["run"].concat(command.split(" ")));
      ...
      if (this.packageInstallSuccess) {
        var formattingSpinner = log.spinner(log.withBrand("Formatting your code")).start();
        var prettierResult = runLocalNodeCLI("prettier --loglevel silent --write .");

        if (prettierResult.status !== 0) {
          formattingSpinner.fail(chalk.yellow.bold("We had an error running Prettier, but don't worry your app will still run fine :)"));
        } else {
      ...
      if (this.options.skipInstall) {
        ...
        spinner.fail(chalk.red.bold("We had some trouble connecting to the network, so we'll skip installing your dependencies right now. Make sure to run " + (this.pkgManager + " install") + " once you're connected again."));
      }
    }
  ...
  _proto.commitChanges = function commitChanges() {
    var commitSpinner = log.spinner(log.withBrand("Committing your app")).start();
    ...
    for (var _i = 0, _commands = commands; _i < _commands.length; _i++) {
      ...
      var result = spawn.sync.apply(spawn, command);

      if (result.status !== 0) {
        commitSpinner.fail(chalk.red.bold("Failed to run command " + command[0] + " with " + command[1].join(" ") + " options."));
        return;
      }
    }
  ...
  _proto.updateForms = function updateForms() {
...
var Field = /*#__PURE__*/function () {
  ...
  Field.parse = function parse(input, schema) {
    ...
    try {
      ...
      if (err instanceof MissingFieldNameError) {
        throw new Error("Each field in a model must have a name, but you supplied " + log.variable(input) + ".\n         Try giving the field as a " + log.variable("name:type") + " pair, such as " + log.variable("description:string") + ".");
      }
  ...
  function Field(name, options) {
    ...
    if (!options.type) {
      baseLogger({
        displayDateTime: false
      }).warn("No field type specified for field " + log.variable(name) + ", falling back to " + log.variable("String") + ".");
...
var getTemplateRoot = function getTemplateRoot(templateDir, fallback) {
  if (!templateDir) {
    return fallback;
  }

  var templatePath = join(templateDir, fallback.path);

  if (!fs.existsSync(templatePath)) {
    baseLogger({
      displayDateTime: false
    }).info("Template path \"" + templatePath + "\" does not exist. Falling back to the default template.");
    return fallback;
  }
...
var ModelGenerator = /*#__PURE__*/function (_Generator) {
  ...
  function ModelGenerator(options) {
    ...
    _this.sourceRoot = getTemplateRoot(options.templateDir, {
      type: "absolute",
      path: ""
    });
    return _this;
  ...
  _proto.write = async function write() {
    var schemaPath = path__default.resolve("db/schema.prisma");
    ...
    try {
      schema = getSchema(this.fs.read(schemaPath));
    } catch (err) {
      baseLogger({
        displayDateTime: false
      }).error("Failed to parse db/schema.prisma file");
      throw err;
    }
    ...
    if (!dryRun) {
      ...
      try {
        if (model) {
          ...
          this.fs.write(schemaPath, printSchema(schema));
    ...
    if (model) {
      newline();
      log.success("Model '" + modelDefinition.name + "'" + (dryRun ? "" : " " + updatedOrCreated + " in schema.prisma") + ":\n");
      printSchema({
        type: "schema",
        list: [model]
      }).split("\n").map(log.progress);
  ...
  _proto.postWrite = async function postWrite() {
    ...
    if (shouldMigrate) {
      await new Promise(function (res, rej) {
        ...
        var child = spawn$1(prismaBin, ["migrate", "dev"], {
          stdio: "inherit"
        });
...
var MutationGenerator = /*#__PURE__*/function (_Generator) {
  ...
  function MutationGenerator(options) {
    var _this;
...
var PageGenerator = /*#__PURE__*/function (_Generator) {
  ...
  _proto.postWrite = async function postWrite() {
    ...
    var _await$import2 = await import('next/dist/build/routes'),
        saveRouteManifest = _await$import2.saveRouteManifest;

    var config = loadConfigProduction(process.cwd());
...
var QueriesGenerator = /*#__PURE__*/function (_Generator) {
  ...
  function QueriesGenerator(options) {
    var _this;
...
export { AppGenerator, ConflictChecker, FormGenerator, Generator, ModelGenerator, MutationGenerator, MutationsGenerator, PageGenerator, QueriesGenerator, QueryGenerator, capitalize, customTsParser, getLatestVersion, pluralCamel, pluralPascal, singleCamel, singlePascal, uncapitalize };
